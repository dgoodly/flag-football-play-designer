...
export default function FlagFootballDesigner() {
  ...
  const [selectedFormation, setSelectedFormation] = useState('gunEmptyBunchRight')
  ...
  const redrawCanvas = () => {
    ...
    // Second set: 35% from edges
    const leftOffset2 = canvas.width * 0.35
    const rightOffset2 = canvas.width - canvas.width * 0.35 - hashWidth
    for (let y = 0; y < canvas.height; y += verticalSpacing) {
      if (!isNearFieldLine(y)) {
  ...
  }, [selectedFormation, erasing, canvasSize])
